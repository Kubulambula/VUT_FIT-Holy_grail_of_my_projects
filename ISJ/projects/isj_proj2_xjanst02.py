...
def solfege(title_hymn):
    ...
    # the input string partitioned to the title (if given) and the actual hymn (I have no idea what sep is supposed to be so I just used it as a helper variable)
    possible_title, sep, hymn = sep[0] if len(sep := title_hymn.split(": ")) == 2 else "", sep, sep[len(sep) - 1]                # vase reseni
